...
2. Confirm the defaults for `SEND_EMAIL_FROM_ADDRESS` (set to `noreply@cwllll.com`), `SEND_EMAIL_FROM_NAME`, and the `allowed_sender_addresses` list in `wrangler.jsonc` (both the root `vars` block and the `env.preview.vars` overrides) align with an approved sender identity for your account, adjusting them if necessary.
